...
STUDIO = [
    ...
    "Kaistraße 18 · 20457 Hamburg · Deutschland",
...
def new_canvas(path: Path, width: float, height: float, title: str) -> Canvas:
    ...
    canvas.setAuthor("Studio Linden")
    return canvas
...
def create_flight(path: Path) -> None:
    ...
    canvas.drawString(44, height - 43, "NORD / AIR")
    canvas.setFont("Helvetica", 8.2)
    canvas.drawString(45, height - 62, "NordAir AG · nordair.example")
    ...
    canvas.setFont("Helvetica", 8)
    canvas.drawString(176, y - 43, "07:10  ·  NordAir 714")
    canvas.setFont("Helvetica-Bold", 15)
    ...
    canvas.setFont("Helvetica", 8)
    canvas.drawString(176, y - 77, "18:20  ·  NordAir 715")
    canvas.setFont("Helvetica-Bold", 15)
    ...
    canvas.drawString(45, 151, "Zahlung vor Abflug · Kartenreferenz TRX-0809")
...
def create_taxi(path: Path) -> None:
    ...
    y = receipt_line(canvas, y, "Fahrzeug", "Taxi 17", width, size=7.4)
    y -= 4
    ...
    y -= 24
    draw_wrapped(canvas, "Aussteller: Nordstern Mobilität GmbH · nordstern-mobil.example · USt-ID DE00 000 001", 19, y, width - 38, size=6.3, leading=8, color=MUTED, font="Courier")
...
def create_hotel(path: Path) -> None:
    ...
    canvas.setFillColor(INK)
    y = draw_lines(canvas, ["Mara Winter", "Studio Linden · Design & Web", "Kaistraße 18 · 20457 Hamburg"], 45, y - 18, leading=12, size=8.7)
    ...
    canvas.drawString(45, 134, "Aussteller: Lumenhof Hotel München · lumenhof.example · USt-ID DE00 000 002")
    draw_footer(canvas, width)
    canvas.save()
...
def create_restaurant(path: Path) -> None:
    ...
    y -= 24
    draw_wrapped(canvas, "Aussteller: Wandelpunkt Küche · USt-ID DE00 000 003", 19, y, width - 38, size=6.2, leading=8, color=MUTED, font="Courier")
    draw_footer(canvas, width, y=13, color=MUTED)
    canvas.save()
...
def create_local_ticket(path: Path) -> None:
    ...
    y -= 21
    draw_wrapped(canvas, "Aussteller: IsarMobil Verkehrsbetriebe GmbH · USt-ID DE00 000 004", 19, y, width - 38, size=6.3, leading=8, color=MUTED)
...
def create_airport_ticket(path: Path) -> None:
    ...
    y -= 17
    draw_wrapped(canvas, "Aussteller: Morgenstern Schiene GmbH · USt-ID DE00 000 005", 19, y, width - 38, size=6.3, leading=8, color=MUTED)
    draw_footer(canvas, width, y=13)
    canvas.save()
