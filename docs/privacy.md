...
- the SQLite database with bookings, file metadata, the activity log, the agent run log (model, token counts, the exchange without file contents), the chat conversations (the exchange without file contents, files referenced by their archive entry) and settings
- `Archiv/` with the imported and attached original documents, named by their SHA-256 hash
...
Pfennig contacts OpenAI only when the user drops a file, when the user sends a chat message, when the user checks the connection in the settings, and for nothing else. Requests ask OpenAI not to store responses (`store: false`); the conversation is kept on the Mac and sent again in full with every request. A file run sends:
...
A chat message sends the same profile, schema and tool results, the message with any attached files, and the earlier messages, tool calls, answers and attached files of that conversation, read again from the archive.
...
Bookkeeping and documents remain under the user's control in the local folder above. Deleting a booking also removes archived originals no other booking references. Deleting a chat conversation removes its stored exchange; files attached in it stay in the archive. Clearing the API key in the settings deletes it from the macOS Keychain. Data already sent to OpenAI is governed by the API account's retention controls.
