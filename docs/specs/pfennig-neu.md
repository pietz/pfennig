...
**Ergänzung Dateien (2026-09-16):** Eine abgelegte Datei wird zuerst gespeichert: Hash, Kopie im Archiv als `<sha256>.<endung>`, Zeile in `dateien`. Existiert der Hash bereits und ist ein Lauf dazu gelungen, ist die Datei fertig („bereits vorhanden“); eine gespeicherte Datei ohne gelungenen Lauf läuft erneut, aus der Inbox oder nach erneutem Ablegen. Dann startet der Agentenlauf mit der Nachricht „Datei 3 hinzugefügt: rechnung.pdf“. `dateien` hat eine hochzählende `id` als Schlüssel und `sha256` als eindeutige Spalte; eine Spalte für die Art der Datei gibt es nicht, ob eine Datei Beleg oder Kontoauszug ist, ergibt sich daraus, ob eine Buchung sie trägt. `buchungen.belege` ist eine JSON-Liste von Datei-IDs und wird vom Agenten gepflegt, nicht von der Anwendung. Prüfregel: Jede ID in `belege` existiert in `dateien`. Das Entfernen eines Belegs von einer Buchung löscht die Datei nicht. Löscht der Nutzer eine Buchung, gehen die Belege mit, die keine andere Buchung mehr trägt: Zeile in `dateien` und Archivkopie (Ergänzung 2026-09-21: was der Nutzer nicht sehen kann, darf ihn nicht blockieren; ohne Dateien-Ansicht existiert ein Beleg nur durch seine Buchung). Dieselbe Datei läuft danach beim Ablegen frisch. Kontoauszüge trägt keine Buchung; sie bleiben bekannt, auch wenn ihre Zahlungsbuchungen gelöscht werden, eine benannte Lücke bis zu einer Dateien-Ansicht. Eine Datei, deren Hash bekannt ist, geht nicht erneut zum Agenten. Die Inbox bleibt Wiederholungsquelle bis zum erfolgreichen Lauf; bei einem Fehler bleiben Archivkopie und `dateien`-Zeile bestehen, und der erneute Versuch verwendet sie. Aus dem Lauf wird eine Buchung nur dann ein Beleg, wenn der Agent die Datei-ID einträgt; bei einem Kontoauszug trägt er sie nicht ein. Damit beginnt der zweite Schritt aus Abschnitt 1, die Kontoauszüge, als Änderung der Anleitung, nicht des Mechanismus: Der Agent ordnet jede Bewegung einer bestehenden Buchung zu und trägt die Zahlung ein, legt Bewegungen ohne Buchung als `nur_zahlung` und private oder interne als `ignoriert` an und prüft vorher, was schon da ist. Die Obergrenze der Werkzeugaufrufe je Lauf liegt bei 60, weil ein Auszug mehr Aufrufe braucht als ein Beleg; sie bleibt ein Netz gegen Endlosschleifen, kein Arbeitslimit.

**Ergänzung Anlagevermögen (2026-09-17):** Ein Anlagegut ist die Kaufbuchung selbst. `buchungen` bekommt die Spalte `nutzungsdauer_jahre` (ganzzahlig, leer heißt gewöhnliche Ausgabe). Ist sie gesetzt, gilt die Buchung als abnutzbares bewegliches Wirtschaftsgut: Anschaffungskosten sind das Netto (beim Kleinunternehmer das Brutto) ohne Privatanteil, Anschaffungsdatum ist `datum`; eine Buchung ist ein Anlagegut, zwei Geräte auf einer Rechnung werden zwei Buchungen. Die Nutzungsdauer ist Wissen, nicht Verhalten, und liegt deshalb in einer Tabelle, die der Agent lesen kann: `afa_tabelle` mit `fundstelle` (Schlüssel), `bezeichnung`, `nutzungsdauer_jahre` und `quelle`, die amtliche AfA-Tabelle für allgemein verwendbare Anlagegüter (BMF vom 15.12.2000, gemeinfrei) vollständig übernommen. Die Tabelle steht im Schema wie jede andere; ihre Zeilen liefert die App mit, als CSV-Datei `Sources/Core/Resources/afa_tabelle.csv` im Bundle, und schreibt sie bei jedem Start neu in die Nutzerdatenbank (alles löschen, alles einfügen). Ein App-Update bringt so korrigierte Zeilen beim nächsten Start mit, ohne Migration; die Tabelle gehört der App, Nutzerdaten stehen nie darin. Weitere Wissenstabellen folgen demselben Weg. Die Tabelle zeigt den heute geltenden Stand, nicht die Historie: wo ein späteres BMF-Schreiben eine Zeile ändert, steht der neue Wert mit dem Schreiben als `quelle`, so bei Computerhardware und Peripherie mit einem Jahr (BMF vom 22.02.2022). Der SQL-Autorisierer gibt die Tabelle lesend frei. Die Anleitung bekommt eine Regel: Ein Gegenstand über 800 Euro netto, der länger als ein Jahr genutzt wird, bekommt `nutzungsdauer_jahre` aus `afa_tabelle`. Keine Prüfregel zwingt ihn; die einzige Prüfregel: `nutzungsdauer_jahre` nur bei Ausgaben und größer null. Die Vorsteuer bleibt unberührt und zählt wie bisher im Zahlungszeitraum. In der EÜR steht ein Anlagegut nicht auf seiner Kategoriezeile, sondern die AfA des Jahres auf Zeile 34 (AfA auf bewegliche Wirtschaftsgüter), unabhängig von der Zahlung (§4 Abs. 3 Satz 3 EStG): linear, Jahresbetrag gleich Anschaffungskosten durch Jahre, im ersten Jahr anteilig nach Monaten ab dem Anschaffungsmonat, das letzte Jahr nimmt den Rest; bei einem Jahr Nutzungsdauer der volle Betrag im Anschaffungsjahr (BMF vom 22.02.2022). Der EÜR-Export bekommt einen zweiten Block für die Anlage AVEÜR: alle Anlagegüter in der Gruppe „Büroausstattung“ (Zeilen 48 bis 54: Anschaffungskosten, Buchwert zu Beginn, Zugänge, AfA, Abgänge, Buchwert am Ende) und die Summe in Zeile 63, dazu eine Einzelliste je Anlagegut mit Titel, Anschaffungsdatum, Kosten, AfA des Jahres und Restbuchwert für die eigenen Unterlagen. Der Inspector zeigt bei Ausgaben ein Feld „Nutzungsdauer in Jahren“; die Tabelle ändert sich nicht. Altbestände aus einem vorhandenen Anlageverzeichnis kommen über den normalen Eingang: das Verzeichnis wird als Datei abgelegt, der Agent legt je Anlagegut eine Buchung mit `art = sonstiges`, dem Original-Anschaffungsdatum, einer Zahlung am Kaufdatum und der Datei als Beleg an. Bewusst nicht unterstützt und im Export mit einem Satz gesagt: Fahrzeuge (Gruppe Kfz, private Nutzung), Gebäude und Grundstücke, immaterielle Wirtschaftsgüter und Software als Anlage (Abonnements sind Aufwand), Verkauf und Privatentnahme eines Anlageguts (Zeile 39, Zeile 19), degressive AfA und Sonderabschreibung nach §7g, Sammelposten, nachträgliche Anschaffungskosten. Ein Anlagegut mit Privatanteil geht mit dem betrieblichen Anteil ein, wie jede Ausgabe; die Nutzungsentnahme für Wirtschaftsgüter im Betriebsvermögen bleibt eine dokumentierte Lücke.
...
- `id` (hochzählende Ganzzahl), `richtung` (einnahme/ausgabe), `art` (rechnung, beleg, gutschrift, steuerzahlung, nur_zahlung, ignoriert, sonstiges), `datum` (Belegdatum), `belegnummer` (optional, wie auf dem Beleg angegeben), `faelligkeit` (optional, gültiges Datum), `titel`, `kategorie` (feste EÜR-Kategorienliste im Code, Schlüssel unwiderruflich), `privatanteil_prozent`, `nutzungsdauer_jahre` (optional, macht die Buchung zum Anlagegut, siehe Ergänzung Anlagevermögen), `notizen`
...
**Inspector.** Rechts, standardmäßig ausgeblendet. Er zeigt alle Informationen eines Eintrags, die nicht in eine Tabelle gehören: Beleg mit Vorschau, Grunddaten einschließlich der optionalen Belegnummer und Fälligkeit, Beträge, Steuer, Zahlungen, Notizen, bei ungeprüften Einträgen eine Bestätigen-Aktion. Beim Bestätigen prüft die Anwendung die aktuell gespeicherte Buchung mit denselben Prüfregeln wie der Agent; bei Fehlern bleibt der Prüfstatus unverändert und die Meldung erscheint im bestehenden Fehlerhinweis. Unvollständige Entwürfe dürfen weiterhin automatisch gespeichert werden. Eine Fälligkeit ist als Überfällig markiert, wenn sie vor dem lokalen heutigen Datum liegt und die Buchung nicht vollständig bezahlt ist. Umsetzung als `.inspector` mit einem Formular im Stil `.grouped`. Alle Abschnitte sind flach und immer sichtbar, keine Akkordeons; ein leerer Abschnitt wird weggelassen, nicht eingeklappt.
...
Rechts „Fristen“ mit den drei dringendsten Zeiträumen, die der Nutzer noch schuldet. Der nächste UStVA-Zeitraum und das nächste EÜR-Jahr sind immer geschuldet; ein früherer Zeitraum nur, wenn eine Buchung in ihn fällt. Bei der EÜR gehört ein Anlagegut außerdem in jedes Jahr seiner laufenden AfA, einschließlich des letzten Teiljahrs; für die UStVA bleiben Beleg- und Zahlungsdaten maßgeblich. Bereits als exportiert vermerkte Zeiträume fehlen, früheste Frist zuerst; die Karte zeigt davon höchstens drei. Jede Zeile trägt einen einzeiligen Namen, das Fälligkeitsdatum und die verbleibende Zeit als Ampel: rot ab überschritten, gelb von drei Tagen bis heute, grün darüber. Ein Klick öffnet das Export-Sheet auf diesem Zeitraum.
...
Berechnung (Ist-Versteuerung nach Zahlungsdatum, Vorsteuer, Reverse Charge, Kleinunternehmer, geprüfte Kennzahlen) und XML-Exporter werden aus dem alten Code übernommen. Die Vorsteuer in Kz 66 zählt nur den betrieblichen Anteil, wie die EÜR den Aufwand; liegt die betriebliche Nutzung unter zehn Prozent, gibt es keinen Abzug (§15 Abs. 1 Satz 2 UStG). Bei Reverse-Charge-Dienstleistungen zählt Kz 67 ebenfalls nur den betrieblichen Anteil der berechneten Steuer; Bemessungsgrundlage und geschuldete Steuer bleiben vollständig. Die Zehn-Prozent-Grenze wird dort nicht angewandt, weil sie Gegenstände betrifft; Kleinunternehmer erhalten weiterhin keinen Kz-67-Abzug. Fristen fallen auf den 10. des Folgemonats bzw. den 31. Juli des Folgejahres und rücken auf den nächsten Werktag, wenn dieser Tag ein Wochenende oder ein bundesweiter Feiertag ist (§108 Abs. 3 AO); regionale Feiertage wie Fronleichnam bleiben außen vor. Mit dem Export kommt eine kleine Tabelle `zeitraeume` (jahr, art, index, exportiert_am), damit die App anstehende Zeiträume erinnern und nachträgliche Änderungen in exportierten Zeiträumen warnen kann. Keine Übermittlung aus der App.
