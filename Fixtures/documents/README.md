...
| 06 | `06-us-software-usd-reverse-charge` | US software vendor invoice in USD 1,000.00, no VAT shown, explicitly states no EUR equivalent. Exercises foreign-currency **reverseCharge** (§13b applies to third-country digital services too) with `missingFields: ["statedEurEquivalent"]`. |
| 07 | `07-uk-consultancy-gbp-reverse-charge` | UK (third-country) consultancy invoice in GBP with a stated EUR equivalent and an explicit reverse-charge note. Exercises **reverseCharge** with `statedEurEquivalent` present (§16 Abs. 6 UStG foreign-currency handling). |
| 08 | `08-hardware-laptop-asset-candidate` | German laptop purchase, net 1,850.00 + 19% VAT. Exercises the **GWG asset threshold** (net > 800 EUR in a hardware category -> `assetCandidate: true`, category `hardware_equipment`). |
| 09 | `09-hardware-monitor-small` | German monitor purchase, net 299.00 + 19% VAT, same vendor as #08. Exercises a hardware purchase **below** the GWG threshold (`hardware_small`, `assetCandidate: false`) for contrast with #08. |
