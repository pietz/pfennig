...
public extension Cent {
    ...
    init?(text: String) {
        ...
        let value = euro * 100 + cent
        self.init(negativ ? -value : value)
    }
}
...
public extension LocalDate {
    ...
    init?(deutsch text: String) {
        ...
        guard parts.count == 3, parts[2].count == 4,
              let tag = Int(parts[0]), let monat = Int(parts[1]), let jahr = Int(parts[2])
        else { return nil }
...
public struct Euroformat: ParseableFormatStyle, Codable, Hashable, Sendable {
    ...
    public func format(_ value: Cent) -> String {
        value.formatted
    }
}
...
public struct Euroeingabe: ParseStrategy, Codable, Hashable, Sendable {
    ...
    public func parse(_ text: String) throws -> Cent {
        guard let value = Cent(text: text) else { throw InputError.betrag }
        return value
    }
}

/// The same for a date as `TT.MM.JJJJ`.
public struct DateFormat: ParseableFormatStyle, Codable, Hashable, Sendable {
    public var parseStrategy = DateInput()

    public init() {}

    public func format(_ value: LocalDate) -> String {
        value.formatted
    }
}

public struct DateInput: ParseStrategy, Codable, Hashable, Sendable {
    public init() {}

    public func parse(_ text: String) throws -> LocalDate {
        guard let datum = LocalDate(deutsch: text) else { throw InputError.datum }
        return datum
    }
}
...
public extension FormatStyle where Self == DateFormat {
    static var deutsch: DateFormat {
        DateFormat()
    }
}
