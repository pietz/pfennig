...
public enum Aufteilung {
    ...
    public static func aufteilen(positionen: [Position], betraege: [Cent]) -> [[Anteil]] {
        let gewichte = positionen.flatMap { [$0.netto.value, $0.steuer.value] }
        ...
        for betrag in betraege {
            ...
            result.append(positionen.enumerated().map { stelle, position in
                Anteil(
    ...
    static func verteilen(gesamt rohGesamt: Int64, gewichte rohGewichte: [Int64]) -> [Int64] {
        ...
        var parts = [Int64](repeating: 0, count: gewichte.count)
        var reste = [Int64](repeating: 0, count: gewichte.count)
        for (stelle, gewicht) in gewichte.enumerated() {
            (parts[stelle], reste[stelle]) = geteilt(gesamt, gewicht, durch: nenner)
        }

        var rest = gesamt - parts.reduce(0, +)
        if rest != 0 {
            let schritt: Int64 = rest > 0 ? 1 : -1
            let reihenfolge = reste.indices.sorted { left, right in
                guard reste[left] == reste[right] else {
                    return schritt > 0 ? reste[left] > reste[right] : reste[left] < reste[right]
                }
                return left < right
            }
            var stelle = 0
            while rest != 0 {
                parts[reihenfolge[stelle % reihenfolge.count]] += schritt
                rest -= schritt
                stelle += 1
            }
        }

        return gespiegelt ? parts.map { -$0 } : parts
    }
