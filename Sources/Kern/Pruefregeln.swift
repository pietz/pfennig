...
public enum Pruefregeln {
    ...
    public static func pruefen(_ buchung: Buchung, profil: Profil) -> [String] {
        alle.compactMap { $0(buchung, profil) }
    }
    ...
    static let steuerPasstZumSatz: Regel = { buchung, _ in
        for (nummer, position) in buchung.positionen.enumerated() {
            ...
            let abweichung = Cent(abs((position.steuer - erwartet).wert))
            guard abweichung > toleranz else { continue }
            return """
            Position \(nummer + 1): steuer \(position.steuer.wert) passt nicht zu netto \
            \(position.netto.wert) bei \(position.steuersatz) Prozent, erwartet \(erwartet.wert) Cent.
            """
        }
        return nil
    }
    ...
    static let datumLiegtNichtWeitInDerZukunft: Regel = { buchung, _ in
        let grenze = Datum(Date().addingTimeInterval(Double(vorlaufTage) * 86400))
    ...
    static let kleinunternehmerNurBeiEigenenEinnahmen: Regel = { buchung, profil in
        ...
        guard profil.kleinunternehmer else {
    ...
    static let zahlungenSindPlausibel: Regel = { buchung, _ in
        for zahlung in buchung.zahlungen where zahlung.betrag <= .null {
            return "Eine Zahlung hat den Betrag \(zahlung.betrag.wert); Zahlungen brauchen einen Betrag über null."
        }
        return nil
    }
}
