...
- **Small by choice.** Build only the agreed scope. Support evidenced cases by composing existing primitives before adding dedicated features or declaring them unsupported. Prefer manual correction of rare cases over extra rules, options, fallbacks or abstractions. Do not prepare for hypothetical extensions or invent missing facts.
...
- **One booking per distinct transaction.** A document may support several bookings sharing its receipt ID. Positions and payments are JSON lists in the booking, not separate entities. Separately depreciated assets each need their own purchase booking. Category and private share apply to the whole booking; mixed positions within one transaction use the dominant category, not separate bookings merely for different categories. Category keys are permanent.
