...
- **Native macOS.** Standard SwiftUI and AppKit behavior. One window: left navigation, table, inspector with flat always-visible sections, toolbar. Settings in the standard settings window. No start page or review page.
- **Begrenzte freigegebene Ergänzung (2026-09-15).** Für den lokalen Vergleich gibt es genau die Sidebar-Einträge „Buchungen“ (bestehende Live-Oberfläche), „Designvorschau“ (isolierte native SwiftUI-Beispieldaten) und „Buchungen · verfeinert“ (zweite isolierte native SwiftUI-Variante mit denselben fiktiven Daten). Das ist keine allgemeine Neugestaltung; zusätzliche Start-, Prüf-, Chat- oder Steuerseiten bleiben ausdrücklich ungebaut.
