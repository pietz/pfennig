...
- **Native macOS.** Standard SwiftUI and AppKit behavior. One window with the single sidebar item „Buchungen“, the live ledger table, inspector with flat always-visible sections, and toolbar. The ledger uses the approved refined native treatment: compact 40-point rows, small neutral category symbols, quiet derived review statuses, and green income amounts with primary-color expenses. Settings stay in the standard settings window. No start page or review page.
