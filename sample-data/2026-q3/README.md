...
4. Die beiden PDFs aus `bank/` nicht importieren: Seit dem 22. September verarbeitet der automatische Eingang nur Rechnungen und Belege. Für den vollständigen Demo-Zahlungsstand die sieben dort aufgeführten Eingänge im Inspector bei den vorhandenen Ausgangsrechnungen eintragen. Die gefilterten Bankübersichten bleiben als Unterlagen für den späteren Chat-Abgleich erhalten.
5. Importergebnisse mit den Soll-Daten in `sources/` vergleichen und die Buchungen im Inspector prüfen. Die Soll-Daten sind keine Zusicherung, dass der Agent jedes Dokument fehlerfrei interpretiert. Erwartet sind 26 Buchungen aus den 26 Belegdateien. Nach Eintragen der sieben Eingänge bleiben eine Website-Rechnung und die Rechnung der Texterin offen.
