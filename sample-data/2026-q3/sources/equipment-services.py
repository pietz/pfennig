...
RECIPIENT = ["Studio Linden · Design & Web", "Mara Winter", "Musterufer 18", "20457 Hamburg", "Deutschland"]
...
def footer(c: Canvas, supplier: str, domain: str) -> None:
    ...
    text(c, M, 23, "Fiktiver Musterbeleg · Nicht zur Zahlung", 7.2, MUTED)
...
def new_pdf(path: Path, title: str) -> Canvas:
    c = Canvas(str(path), pagesize=A4); c.setTitle(title); c.setAuthor("Fiktive Beispieldaten Pfennig"); return c
...
def mobile_pdf(path: Path, items: list[dict]) -> None:
    ...
    text(c, 330, H - 288, "Leistungszeitraum 01.08. - 31.08.2026", 8.5, MUTED); text(c, 330, H - 302, "Rufnummer · 040 555 018 70 (fiktiv)", 8.5, MUTED)
...
def laptop_pdf(path: Path, items: list[dict]) -> None:
    ...
    text(c, M + 18, H - 327, "Mobiler Arbeitsplatz · 14 Zoll · 16 GB · 512 GB SSD", 9, gray); text(c, M + 18, H - 350, "Artikel-Nr. PK-SB14-26 (fiktiv)", 8, gray)
