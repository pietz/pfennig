...
func datum(_ jahr: Int, _ monat: Int, _ tag: Int) -> Datum {
    Datum(jahr: jahr, monat: monat, tag: tag)
}
...
func buchung(
    ...
    Buchung(
        ...
        zahlungen: zahlungen.enumerated().map { stelle, zahlung in
            var nummeriert = zahlung
            nummeriert.id = stelle + 1
            return nummeriert
        }
    )
}
...
extension UStVA {
    ...
    func betrag(_ nummer: Int) -> Int64 {
        zeilen.first { $0.kennzahl.nummer == nummer }?.betrag.wert ?? 0
    }
